...
# YOUR CODE HERE
tokens_rdd = pairs_rdd     .map(lambda x: (x[0], set(tok.tokenize(x[1]))))     .groupByKey()     .flatMap(lambda x: [(x[0], token) for token in set.union(*x[1])]).cache()
freq_tokens_rdd = tokens_rdd.map(lambda x:(x[1],1)).reduceByKey(lambda x, y: x+y).cache()
num_of_tokens = freq_tokens_rdd.count()
# YOUR CODE ENDS
...
# YOUR CODE HERE
freq_tokens_rdd = freq_tokens_rdd.filter(lambda x:x[1]>=100)
...
# popular_10_in_each_group_rdd = tokens_rdd.filter(lambda x:x[1] in freq_tokens_map)\
#                     .map(lambda x:((partition.get(x[0],7), x[1]), 1))\
#                     .reduceByKey(lambda x,y :x+y)\
#                     .map(lambda x: (x[0][0], (x[0][1], get_rel_popularity(x[1],freq_tokens_map[x[0][1]]))))\
#                     .partitionBy(8).map(lambda x:x[1])
# popular_10_in_each_group = popular_10_in_each_group_rdd.glom().map(lambda x: sorted(x,key=lambda a: (-a[1],a[0])))\
...
if _is_in_develop:
    get_ipython().system('jupyter nbconvert --to script HW3.ipynb --output HW3')
